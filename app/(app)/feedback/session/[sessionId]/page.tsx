...
interface XPResult { xp_gained: number; total_xp: number; level: number; level_up: boolean; streak: number; new_badges: string[] }
...
export default function SessionFeedbackPage() {
  ...
  useEffect(() => {
    ...
    // Read XP result saved by session page
    const stored = sessionStorage.getItem(`xp_${sessionId}`);
    if (stored) { try { setXpResult(JSON.parse(stored)); } catch {} }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <BadgeToast newBadgeIds={xpResult?.new_badges ?? []} />
